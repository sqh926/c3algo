...
| [`c3algo::collections::avltree`](src/collections/avltree.c3)                 | A sorted set that also answers the $k$-th smallest key, the number of elements with key $\le x$ and the two neighbours of $x$, in $O(\log n)$.                                              |
